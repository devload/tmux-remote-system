...
import com.sudoku.service.HitoriService;
...
@RestController
@RequestMapping("/api/hitori")
@RequiredArgsConstructor
public class HitoriController {

    private final HitoriService hitoriService;

    /**
     * 게임 시작 - 오늘의 Hitori 퍼즐로 게임 시작
     */
    @PostMapping("/start")
    public ResponseEntity<Map<String, Object>> startGame(@RequestBody Map<String, Long> request) {
        ...
        try {
            Map<String, Object> result = hitoriService.startGame(playerId);
    ...
    public ResponseEntity<Map<String, Object>> checkSolution(@RequestBody Map<String, Object> request) {
        ...
        try {
            Map<String, Object> result = hitoriService.checkSolution(sessionId, solution);
            return ResponseEntity.ok(result);
        } catch (RuntimeException e) {
    ...
    public ResponseEntity<Map<String, Object>> completeGame(@RequestBody Map<String, Object> request) {
        ...
        try {
            Map<String, Object> result = hitoriService.completeGame(sessionId, solution, timeSeconds);
            return ResponseEntity.ok(result);
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    ...
    public ResponseEntity<Map<String, Object>> useHint(@RequestBody Map<String, Object> request) {
        ...
        try {
            Map<String, Object> result = hitoriService.useHint(sessionId);
            return ResponseEntity.ok(result);
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    ...
    public ResponseEntity<Map<String, Object>> checkDailyStatus(@PathVariable Long playerId) {
        Map<String, Object> status = hitoriService.checkDailyStatus(playerId);
    ...
    public ResponseEntity<List<Map<String, Object>>> getTodayRanking(
        ...
        List<Map<String, Object>> ranking = hitoriService.getTodayRanking(limit);
